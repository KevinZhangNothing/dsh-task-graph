...
`;
    ...
    function layoutSnake(graph, vis) {
      ...
      const pos = new Map();
      const HG = 26;   // gap between cards within a row
      const VG = 66;   // gap between rows (room for the wrap edge)
      order.forEach((n, i) => {
    ...
    class GraphRenderer {
      ...
      _drawSnakeEdges(layout, graph, runningIds) {
        ...
        for (let i = 0; i + 1 < order.length; i += 1) {
          ...
          this.edgeLayer.appendChild(svgEl('path', {
            d, class: cls.join(' '), 'marker-end': 'url(#tg-arrow)', 'data-edge': `snake-${i}`,
          }));
        }
