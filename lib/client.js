...
`;
  ...
  // Register with the DSH client module loader when present (so the boot
  // manifest of an installed profile can materialize it), AND run directly:
  // `load()` only registers — materialization happens on import, so runtime
  // injection (dev pages) needs the direct call. The mount guard in boot()
  // makes double execution harmless.
