...
.tg-edge.tg-edge-snake{stroke:color-mix(in srgb, var(--dsw-alias-border-l3,#a8b6cc) 68%, transparent);stroke-width:2}
.tg-edge.tg-edge-snake.tg-edge-flow{stroke:color-mix(in srgb, var(--tg-accent) 75%, transparent)}
.tg-arrowhead.tg-snake{fill:color-mix(in srgb, var(--dsw-alias-border-l3,#a8b6cc) 80%, transparent)}
...
`;
    ...
    /* ================================================================ *
     * Snake layout — up to 6 key nodes per row, left→right, wrap down,  *
     * right→left, and so on (boustrophedon execution narrative).        *
     * ================================================================ */
    const SNAKE_COLS = 6;

    /** Execution-narrative order: DFS over the containment tree, children in
     *  time order — task, turn, its key events, next turn, … */
    function orderForSnake(graph, vis) {
      const childrenOf = new Map();
      const byId = new Map();
      for (const n of vis.visible) byId.set(n.id, n);
      ...
      const visit = (n) => {
        ...
        seen.add(n.id);
        ordered.push(n);
        const kids = (childrenOf.get(n.id) ?? []).slice()
          .sort((a, b) => (a.start_time ?? 0) - (b.start_time ?? 0));
        for (const k of kids) visit(k);
      };
      const roots = vis.visible
        .filter((n) => !n.parent_id || !byId.has(n.parent_id))
        .sort((a, b) => (a.start_time ?? 0) - (b.start_time ?? 0));
      for (const r of roots) visit(r);
      for (const n of vis.visible) if (!seen.has(n.id)) { seen.add(n.id); ordered.push(n); }
      return ordered;
    }

    function layoutSnake(graph, vis) {
      const order = orderForSnake(graph, vis);
      const pos = new Map();
      const HG = 40;   // gap between cards within a row (room for the spine edge)
      const VG = 88;   // gap between rows (room for the wrap edge)
      order.forEach((n, i) => {
        const row = Math.floor(i / SNAKE_COLS);
        const k = i % SNAKE_COLS;
        const col = row % 2 === 0 ? k : SNAKE_COLS - 1 - k;
        pos.set(n.id, { x: col * (NODE_W + HG), y: row * (NODE_H + VG), w: NODE_W, h: NODE_H });
      });
      const rows = Math.max(1, Math.ceil(order.length / SNAKE_COLS));
      const width = SNAKE_COLS * NODE_W + (SNAKE_COLS - 1) * HG;
      const height = rows * NODE_H + (rows - 1) * VG;
      return { pos, width, height, order, mode: 'snake', dir: 'snake' };
    }
    ...
    class GraphRenderer {
      constructor(canvas, app) {
        ...
        this.arrowSnake = svgEl('marker', {
          id: 'tg-arrow-snake', viewBox: '0 0 10 10', refX: '8.5', refY: '5',
          markerWidth: '8.5', markerHeight: '8.5', orient: 'auto-start-reverse',
        }, [svgEl('path', { d: 'M0,0 L10,5 L0,10 z', class: 'tg-arrowhead tg-snake' })]);
        this.defs.append(this.arrow, this.arrowHot, this.arrowSnake);
      ...
      _bindPanZoom() {
        ...
        this.canvas.addEventListener('wheel', (ev) => {
          ...
          if (ev.ctrlKey || ev.metaKey) {
            // zoom at the cursor (also trackpad pinch)
            const factor = Math.exp(-ev.deltaY * 0.0016);
            const nk = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, this.view.k * factor));
            const scale = nk / this.view.k;
            this.view.x = cx - (cx - this.view.x) * scale;
            this.view.y = cy - (cy - this.view.y) * scale;
            this.view.k = nk;
          } else {
            // document-style scrolling: vertical (horizontal with Shift)
            this.view.y -= ev.deltaY;
            this.view.x -= ev.shiftKey ? ev.deltaY : ev.deltaX;
          }
          this._clampPan();
          this._applyTransform();
        }, { passive: false });
      ...
      fit() {
        ...
        const pad = 40;
        if (this.layout.mode === 'snake') {
          // snake: fill the width at a readable scale, start at the top,
          // then scroll vertically through the rows.
          const k = Math.min((rect.width - pad * 2) / width, 1.1);
          this.view.k = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, k));
          this.view.x = (rect.width - width * this.view.k) / 2;
          this.view.y = pad * 0.5;
        } else {
          const k = Math.min(
      ...
      /** Keep at least part of the graph on screen while panning/scrolling. */
      _clampPan() {
        if (!this.layout) return;
        const rect = this.canvas.getBoundingClientRect();
        const w = this.layout.width * this.view.k;
        const h = this.layout.height * this.view.k;
        const M = 80; // keep this many px of the graph visible
        this.view.x = Math.min(Math.max(this.view.x, -(w - M)), rect.width - M);
        this.view.y = Math.min(Math.max(this.view.y, -(h - M)), rect.height - M);
      }
      ...
      render(graph, vis, layout, opts) {
        ...
        // ---- edges ----
        if (layout.mode === 'snake') this._drawSnakeEdges(layout, graph, runningIds);
        for (const e of layout.mode === 'snake' ? [] : graph.edges) {
      ...
      /** Snake spine: one continuous execution flow through the ordered cards. */
      _drawSnakeEdges(layout, graph, runningIds) {
        const order = layout.order ?? [];
        for (let i = 0; i + 1 < order.length; i += 1) {
          const pa = layout.pos.get(order[i].id);
          const pb = layout.pos.get(order[i + 1].id);
          if (!pa || !pb) continue;
          const sameRow = Math.abs(pa.y - pb.y) < 1;
          let d;
          if (sameRow) {
            const ltr = pb.x > pa.x;
            const sx = ltr ? pa.x + pa.w : pa.x;
            const tx = ltr ? pb.x : pb.x + pb.w;
            const sy = pa.y + pa.h / 2;
            const ty = pb.y + pb.h / 2;
            const mx = (sx + tx) / 2;
            d = `M${sx},${sy} C${mx},${sy} ${mx},${ty} ${tx},${ty}`;
          } else {
            // row wrap: last card of the row -> first card of the next row.
            // bow outward so the turn reads clearly.
            const sx = pa.x + pa.w / 2;
            const sy = pa.y + pa.h;
            const tx = pb.x + pb.w / 2;
            const ty = pb.y;
            const atRightEdge = pa.x > 0; // wrap happens at a grid edge; bow outward
            const bx = atRightEdge ? Math.max(sx, tx) + 36 : Math.min(sx, tx) - 36;
            d = `M${sx},${sy} C${bx},${sy + 42} ${bx},${ty - 42} ${tx},${ty}`;
          }
          const cls = ['tg-edge', 'tg-edge-snake'];
          if (runningIds.has(order[i].id) || runningIds.has(order[i + 1].id)) cls.push('tg-edge-flow');
          this.edgeLayer.appendChild(svgEl('path', {
            d, class: cls.join(' '), 'marker-end': 'url(#tg-arrow-snake)', 'data-edge': `snake-${i}`,
          }));
        }
        // retry loops stay visible as arcs on their cards
        for (const e of graph.edges) {
          if (e.type === 'retries' && e.source === e.target) {
            this._drawRetryArc(layout.pos.get(e.source), e, 'snake');
          }
        }
      }
    ...
    class App {
      ...
      _build() {
        ...
        this.layoutBtns = {};
        for (const mode of ['snake', 'dag', 'timeline', 'force']) {
          const b = el('button', { class: 'tg-btn', onclick: () => this.setLayout(mode) },
            [{ snake: '◆ 蛇形', dag: 'DAG', timeline: '时间线', force: '力导向' }[mode]]);
        ...
        this.expandBtn = el('button', { class: 'tg-btn', title: '折叠全部分组 / 展开全部分组', onclick: () => this.toggleCollapseAll() }, ['⇕ 全部折叠']);
        ...
        const ctrlRow = el('div', { class: 'tg-toprow', style: 'padding-top:0' }, [
          this.keyBtn, this.cpBtn, this.trajBtn,
          el('span', { class: 'tg-spacer' }),
        ...
        this.hint = el('div', { class: 'tg-hint' }, ['滚轮滚动 · Ctrl+滚轮缩放 · 拖拽平移 · 点击节点看详情 · 双击折叠/展开']);
      ...
      /** Keep the collapse-toggle label honest after any render. */
      _syncCollapseBtn() {
        if (!this.expandBtn || !this.viewGraph) return;
        const anyCollapsed = this.viewGraph.nodes.some((n) => this.collapsed.has(n.id));
        this.expandBtn.textContent = anyCollapsed ? '⇕ 全部展开' : '⇕ 全部折叠';
      }
  /** Collapse every group node, or expand everything back — always visible effect. */
      toggleCollapseAll() {
        const g = this.viewGraph;
        if (!g) return;
        const anyCollapsed = g.nodes.some((n) => this.collapsed.has(n.id));
        if (anyCollapsed) {
          this.collapsed.clear();
        } else {
          const parents = new Set();
          for (const n of g.nodes) if (n.parent_id) parents.add(n.parent_id);
          for (const n of g.nodes) {
            if (n.type !== 'tool' && n.type !== 'task' && parents.has(n.id)) this.collapsed.add(n.id);
          }
        }
        this.render();
      }
      ...
      render(fit = false) {
        ...
        else if (this.layoutMode === 'snake') layout = layoutSnake(g, vis);
    ...
    /** The pane's tab bar lives inside a <header>; never touch tablists in
     *  dialogs or other modules (e.g. the @-file settings' scope tablist). */
    function findTablist() {
      let fallback = null;
      for (const tl of document.querySelectorAll('[role="tablist"]')) {
        if (tl.closest('[role="dialog"]')) continue;
        if (tl.closest('header')) return tl;
        // version fallback: native pane tabs carry a *_tab CSS-module class
        if (!fallback && [...tl.querySelectorAll('[role="tab"]')].some((t) => /_tab$/.test(t.className))) fallback = tl;
      }
      return fallback;
    }
    ...
    function mountTabEntry() {
      ...
      const reconcile = () => {
        ...
        const tablist = findTablist();
        if (tablist) {
          // misplaced (hijacked by another module's tablist) -> pull it back
          if (myTab.isConnected && myTab.parentElement !== tablist) myTab.remove();
