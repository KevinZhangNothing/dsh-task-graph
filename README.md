...
![图谱总览（蛇形布局 · 类型色块 · 关键节点）](docs/images/hero.png)
...
![详情面板：批量代码改动可下钻到每一处](docs/images/detail.png)
...
- **DAG**：分层有向图，一键切换「⇄ 横向 / ⇅ 纵向」
- **时间线**：按真实执行时刻铺开，直观看到并行与耗时
- **力导向**：自由探索复杂关系
- **滚轮纵向滚动**浏览长图，`Ctrl/⌘ + 滚轮`在光标处缩放（限幅 0.3×–2.2×，不会缩成看不清的小图）；拖拽平移、⤢ 适配宽度、折叠/展开、类型过滤、搜索、聚焦节点、邻域高亮

![时间线布局](docs/images/timeline.png)
...
![轨迹视图（玻璃透明）](docs/images/trajectory.png)
